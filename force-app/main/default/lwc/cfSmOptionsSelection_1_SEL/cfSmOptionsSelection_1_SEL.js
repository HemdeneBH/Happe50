import { FlexCardMixin } from "vlocity_cmt/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_cmt/flexCardUtility";
          ...
          export default class cfSmOptionsSelection_1_SEL extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfSmOptionsSelection_1_SEL';
 ...
 this.registerEvents();
                this.setAttribute(
                  "class",
                  "card-a3t5q000000oMYVAA2"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5q000004OYWaEAO").then(() =>{
                    //Loading Custom CSS.
                  })
                });
        ...
        this.pubsubEvent[0] = {
          [interpolateWithRegex(`showprice`,this._allMergeFields,this._regexPattern,"noparse")]: this.handleEventAction.bind(this, data.events[0],0)
        };
        this.pubsubChannel0 = interpolateWithRegex(`smFlex_OptionsVertes`,this._allMergeFields,this._regexPattern,"noparse");
